...
filename = "/home/chinmay/Documents/Pyproj/sound.wav"
...
circle_x = int(screen_w / 2)
circle_y = int(screen_h / 2)
...
for g in tmp_bars:
    ...
    for c in g:
        gr.append(
            RotatedAverageAudioBar(circle_x + radius * math.cos(math.radians(ang - 90)), circle_y + radius * math.sin(math.radians(ang - 90)), c, (255, 0, 255), angle=int(ang), width=0, max_height=370)
        )
        ang += angle_dt
...
while running:
    ...
    pygame.draw.circle(screen, circle_color, (circle_x, circle_y), int(radius))
    pygame.display.flip()
pygame.quit()