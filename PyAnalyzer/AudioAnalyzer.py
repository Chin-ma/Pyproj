...
class AudioAnalyzer:
    ...
    def load(self, filename):
        ...
        self.spectogram = librosa.amplitude_to_db(stft, ref=np.max)

        frequencies = librosa.core.fft_frequencies(n_fft=2048*4)

        times = librosa.frames_to_time(np.arange(self.spectogram.shape[1]), sr=sample_rate, hop_length=512, n_fft=2048*4)
    ...
    def show(self):
        librosa.display.specshow(self.spectogram, y_axis='log', x_axis='time') # type: ignore
    ...
    def get_decibel(self, target_time, freq):
        return self.spectogram[int(freq*self.frequencies_index_ratio)][int(target_time*self.time_index_ratio)] # type: ignore
...
class AudioBar:
    def __init__(self, x, y, freq, color, width=50, min_height=10, max_height=100, min_decibel=-80, max_decibel=0):
        self.x, self.y, self.freq = x, y, freq # type: ignore
        self.color = color
...
class Rect:
    ...
    def draw(self, screen):
        pygame.draw.polygon(screen, (255, 0, 0), self.points)